...
public class MainTest extends ApplicationTest {
    ...
    private static final Logger logr = Logger.getLogger(Main.class.getName()); // matches the logger in the affected class
    @Rule public final ExpectedException expectedException = ExpectedException.none();  // expected exception
    private final ResourceBundle bundle = ResourceBundle.getBundle("bundle");  // getting resource bundle
    ...
    @Override
    public void stop() {
        Main.getDBEditor().closeConnection(); // Closing main database connection
    }
    ...
    public void findCurrentOS() {
        Assert.assertNotEquals(bundle.getString("noSupport"), Main.findCurrentOS());
        // Windows
    ...
    @Test
    public void getFileText_NullPath(){
        String file = null,
                expected = "Failed to read file (null) and turn to String.";
        String fileText =  Main.getFileText(file);

        Assert.assertEquals("", fileText);
        Assert.assertTrue(getTestCapturedLog(logCapturingStream, customLogHandler).contains(
                expected));
    }

    @Test
    public void getFileText_EmptyStringPath(){
        String file = "",
                expected = "Failed to read file () and turn to String.";
        String fileText =  Main.getFileText(file);
    ...
    @Test
    public void getFileText_InvalidPath(){
        String file = "invalid/path",
                expected = "Failed to read file (invalid/path) and turn to String.";
        String fileText =  Main.getFileText(file);
    ...
    @Test
    public void getFileText_ValidPath(){
        String fileText = Main.getFileText("/text/test.txt");

        Assert.assertEquals("This File is For Testing A Main Function\n", fileText);
    }
    ...
    @Test
    public void replaceBundleString_TwoReplacementsSameNumber_OneParam() {
        // saving value from replaceBundleString function
        String twoReplacements_twoReplacements = Main.replaceBundleString("replaceBundleString_oneSameReplacements", "ONE");

        // making sure values are what they should be
        Assert.assertEquals("This string contains a replacement here: ONE, and the same replacement here: ONE", twoReplacements_twoReplacements);
    }
    ...
    @Test
    public void replaceBundleString_FourReplacementsTwoSameNumber_TwoParams() {
        // saving value from replaceBundleString function
        String twoReplacements_twoReplacements = Main.replaceBundleString("replaceBundleString_twoSameReplacements", "ONE", "TWO");

        // making sure values are what they should be
        Assert.assertEquals("First: ONE, Second: TWO, Third: ONE, Fourth: TWO", twoReplacements_twoReplacements);
    }
    ...
    @Test
    public void navigateToLink_ProperURL() {  // unsupported on travis ci
        //NOTE: "www.google.com" is valid on Windows but invalid on macOS
        Assert.assertTrue(Main.navigateToLink("https://www.google.com"));
    }
